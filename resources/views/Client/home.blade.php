                                            ...
                                            @foreach ($categories as $category)
                                                <li>
                                                    <button type="button" value="{{ $category->id }}"
                                                        class="category-filter-btn inline-flex w-full px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">{{ $category->title }}</button>
                                                </li>
                                            @endforeach
                                        </ul>
                                    </div>
                                                ...
                                                @else
                                                    ...
                                                    @if ($userReservation)
                                                        @if ($userReservation->status == 'Approved')
                                                            <span
                                                                class="bg-green-100 text-green-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-green-400 border border-green-400">{{ $userReservation->status }}</span>
                                                        @elseif($userReservation->status == 'Pending')
                                                            <span
                                                                class="bg-yellow-100 text-yellow-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-yellow-400 border border-yellow-400">{{ $userReservation->status }}</span>
                                                        @elseif($userReservation->status == 'Rejected')
                                                            <span
                                                                class="bg-red-100 text-red-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-red-400 border border-red-400">{{ $userReservation->status }}</span>
                                                        @endif
                                                    @else
                                                        <div class="flex gap-2">
                                                            <form
                                            ...
                                            <div class="flex flex-col justify-between gap-2 text-sm text-gray-600">
                                                ...
                                                <div class="flex justify-between items-center gap-9 mt-3">
                                                    ...
                                                    <div class="flex gap-4 h-fit">
                                                        <a type="button"
                                                            href="{{ route('moreDetails', ['id' => $event->id]) }}"
                                                            title="View Details"
                                                ...
                                                @if ($userReservations->contains('event_id', $event->id) && $userReservations->contains('status', 'Approved'))
                                                    <div class="flex justify-center">
                                                        @foreach ($userReservations as $userReservation)
                                                            @if ($userReservation->event_id == $event->id && $userReservation->status == 'Approved')
                                                                <a href="{{ route('generateTicket', ['reservation' => $userReservation->id, 'event' => $event->id]) }}"
